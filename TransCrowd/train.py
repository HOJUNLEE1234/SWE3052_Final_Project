...
def main(args):
    ...
    if args['model_type'] == 'token':
        ...
        model = base_patch16_384_gap(pretrained=trace_module)
    ...
    val_data = pre_data(test_list, args, train=False)
    ...
    for epoch in range(args['start_epoch'], args['epochs']):
        ...
        if epoch % 5 == 0:
            val_mae = validate(val_data, model, args)
            end2 = time.time()

            # Check for the best model based on validation performance
            is_best = val_mae < args['best_pred']
            args['best_pred'] = min(val_mae, args['best_pred'])

            print(f"Epoch {epoch} * Best MAE on Validation: {args['best_pred']:.3f}, Time: {end1 - start:.2f}s, Validation: {end2 - end1:.2f}s")
    ...
    test_mae = validate(val_data, model, args)
